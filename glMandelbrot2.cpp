...
// consts
const int WRESX = 1300;
const int WRESY = 1300;
const int ARESX = 1300;
const int ARESY = 1300;
...
void fill(unsigned char arr[ARESY][ARESX][3]) {
	for(int r = 0; r < ARESY; r++) {
		for(int c = 0; c < ARESX; c++) {
			...
			int n = isBounded(C);
			arr[r][c][0] = n/3%256; // red
			arr[r][c][1] = 4*n%256; // green
...
int isBounded(complex <double> C) {
	...
	int i = 0;
	for(i = 0; i < L; i++) {
		M = atanh(C+M*M)+(sinh(M*M + C));//+asinh(23);
